...
abstract class HassService<ST>(
    ...
    private fun loadServiceConfig() {
        ...
        throw CouldNotPerformException(("Could not detect service config! Service[" + serviceType.name + "] is not configured in Unit[" + (unit as Unit<*>).id).toString() + "]!")
    }

    @Throws(NotSupportedException::class)
    fun detectServiceType(): ServiceTemplate.ServiceType {
        return ServiceTemplate.ServiceType.valueOf(
